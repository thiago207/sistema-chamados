@extends('layouts.app')

@section('titulo', 'Matérias · Sale Marketing')

@section('content')

@include('partials.breadcrumb', ['items' => [
    ['label' => 'Grade de Horários', 'url' => '/grade'],
    ['label' => 'Matérias'],
]])

<div class="page-header">
    <div>
        <h1 class="page-header__title">Matérias</h1>
        <p class="page-header__subtitle mb-0">{{ $materias->count() }} matéria(s) cadastrada(s)</p>
    </div>
    <button type="button" class="btn btn-brand" data-bs-toggle="modal" data-bs-target="#modalNovaMateria">
        <i class="bi bi-plus-lg"></i> Nova Matéria
    </button>
</div>

@if(session('sucesso'))
    <div class="alert alert-success d-flex align-items-center gap-2">
        <i class="bi bi-check-circle"></i>
        <span>{{ session('sucesso') }}</span>
    </div>
@endif

<div class="card mb-3">
    <div class="card-body">
        <form action="/grade/materias" method="GET" class="d-flex gap-2">
            <input type="text" name="busca" class="form-control" placeholder="Buscar por nome..." value="{{ $busca }}">
            <button type="submit" class="btn btn-outline-secondary"><i class="bi bi-search"></i></button>
            @if($busca)
                <a href="/grade/materias" class="btn btn-outline-secondary">Limpar</a>
            @endif
        </form>
    </div>
</div>

@php
    $corTipo = fn ($tipo) => match($tipo) {
        'comum_curricular' => 'bg-primary',
        'eletiva' => 'bg-info text-dark',
    };
    $rotuloTipo = fn ($tipo) => match($tipo) {
        'comum_curricular' => 'Comum Curricular',
        'eletiva' => 'Eletiva',
    };
@endphp

@if($materias->isEmpty())
    <div class="card">
        <div class="card-body">
            <div class="empty-state">
                <i class="bi bi-journal-bookmark"></i>
                <strong>Nenhuma matéria cadastrada</strong>
                Cadastre a primeira matéria para começar.
            </div>
        </div>
    </div>
@else
    <div class="card">
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover mb-0 align-middle">
                    <thead>
                        <tr>
                            <th class="ps-4">#</th>
                            <th>Nome</th>
                            <th>Tipo</th>
                            <th class="text-end pe-4">Ações</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($materias as $materia)
                            <tr>
                                <td class="ps-4 text-muted">{{ $materia->id }}</td>
                                <td class="fw-semibold text-brand">{{ $materia->nome }}</td>
                                <td><span class="badge {{ $corTipo($materia->tipo) }}">{{ $rotuloTipo($materia->tipo) }}</span></td>
                                <td class="pe-4">
                                    <div class="d-flex gap-2 justify-content-end">
                                        <button class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#modalEditar{{ $materia->id }}">
                                            <i class="bi bi-pencil"></i> Editar
                                        </button>
                                        <form action="/grade/materias/{{ $materia->id }}" method="POST"
                                            data-confirm="Excluir esta matéria?" data-confirm-tipo="danger">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-sm btn-danger">
                                                <i class="bi bi-trash"></i> Excluir
                                            </button>
                                        </form>
                                    </div>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
            </div>
        </div>
    </div>

    {{-- Modais de edição --}}
    @foreach($materias as $materia)
        <div class="modal fade" id="modalEditar{{ $materia->id }}" tabindex="-1">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header bg-brand text-white">
                        <h5 class="modal-title">Editar Matéria</h5>
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body">
                        <form action="/grade/materias/{{ $materia->id }}" method="POST">
                            @csrf
                            @method('PUT')
                            <div class="mb-3">
                                <label class="form-label">Nome</label>
                                <input type="text" name="nome" class="form-control" value="{{ $materia->nome }}" required>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Tipo</label>
                                <select name="tipo" class="form-select">
                                    <option value="comum_curricular" {{ $materia->tipo === 'comum_curricular' ? 'selected' : '' }}>Comum Curricular</option>
                                    <option value="eletiva" {{ $materia->tipo === 'eletiva' ? 'selected' : '' }}>Eletiva</option>
                                </select>
                            </div>
                            <div class="modal-footer px-0 pb-0">
                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
                                <button type="submit" class="btn btn-brand fw-bold">Salvar</button>
                            </div>
                        </form>
                    </div>
                </div>
            </div>
        </div>
    @endforeach
@endif

{{-- Modal de criação --}}
<div class="modal fade" id="modalNovaMateria" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header bg-brand text-white">
                <h5 class="modal-title">Nova Matéria</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <form action="/grade/materias" method="POST">
                    @csrf
                    <div class="mb-3">
                        <label class="form-label">Nome</label>
                        <input type="text" name="nome" class="form-control {{ $errors->has('nome') ? 'is-invalid' : '' }}" placeholder="Ex: Matemática" value="{{ old('nome') }}" required>
                        @if($errors->has('nome'))
                            <div class="invalid-feedback">{{ $errors->first('nome') }}</div>
                        @endif
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Tipo</label>
                        <select name="tipo" class="form-select">
                            <option value="comum_curricular" {{ old('tipo', 'comum_curricular') === 'comum_curricular' ? 'selected' : '' }}>Comum Curricular</option>
                            <option value="eletiva" {{ old('tipo') === 'eletiva' ? 'selected' : '' }}>Eletiva</option>
                        </select>
                    </div>
                    <div class="modal-footer px-0 pb-0">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
                        <button type="submit" class="btn btn-brand fw-bold">Salvar</button>
                    </div>
                </form>
            </div>
        </div>
    </div>
</div>

@endsection

@section('scripts')
    @if($errors->any())
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                new bootstrap.Modal(document.getElementById('modalNovaMateria')).show();
            });
        </script>
    @endif
@endsection
