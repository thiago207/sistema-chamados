...
<div class="page-header">
    ...
    <div class="d-flex gap-2">
        <a href="/grade/horarios/geral" class="btn btn-outline-secondary">
...
@if(session('naoAlocadas') && count(session('naoAlocadas')) > 0)
    <div class="card mb-3">
        <div class="card-header bg-warning">
            <strong>{{ count(session('naoAlocadas')) }} aula(s) não alocada(s) na última geração</strong>
        </div>
        <ul class="list-group list-group-flush">
            @foreach(session('naoAlocadas') as $item)
                <li class="list-group-item"><small>{{ $item }}</small></li>
            @endforeach
        </ul>
    </div>
@endif
                                ...
                                @for($numero = 1; $numero <= $max; $numero++)
                                    <tr>
                                        ...
                                        @foreach($dias as $dia)
                                            ...
                                            <td>
                                                <button type="button" class="btn btn-sm w-100 {{ $horario ? 'btn-outline-primary' : 'btn-outline-secondary' }} btn-celula"
